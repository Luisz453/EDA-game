...
struct PLAYER_NAME : public Player
{
  ...
  void comer(const vector<int> & mis_soldados)
  {
    ...
    Dir buscar_enemigo(Pos p, Pos comida, const Tablero &objetivo)
  {
    vector<vector<bool>> visitados(board_rows(),vector<bool>(board_cols(), false));
    queue<Pos_y_Dis> cua;
    visitados[comida.i][comida.j] = true;
        ...
        for(Dir i : diagonales ){
          ...
          if(pos_ok(siguiente) and cell(siguiente).type != Waste){
            ...
            if(celda.id != -1){
              ...
              if(us.player == -1){
                if(i == DR){
                  Pos posible = actual + Up;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Up);
                  posible = actual + Left;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Left);
                }
                else if ( i == LD){
                  Pos posible = actual + Up;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Down);
                  posible = actual + Right;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Left);

                }
                else if ( i == UL){
                  Pos posible = actual + Down;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Down);
                  posible = actual + Right;
                  if(pos_ok(siguiente) and cell(siguiente).type != Waste) move(i, Left);
